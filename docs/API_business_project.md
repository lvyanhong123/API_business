...
**适配器模式**：各供应商的接口规范、加解密方式不同，需要通过脚本/适配器来处理。

**当前流程（公司现状）**：
- 开发写代码处理供应商API的对接
- 发版后，运营人员在页面上填写接口地址、入参出参等业务参数
- 这种方式的优点是技术人员只需处理技术问题，运营人员可以独立配置业务参数

**改进思路**：
- 采用Groovy脚本/JavaScript适配器插件，通过热部署方式发布
- 开发人员提供脚本，运营人员通过页面配置业务参数（API地址、入参出参等）
- 密钥写在脚本中，由开发人员管理
...
|------|------|
| 1:1 | 一个供应商对应一个脚本 |
| 密钥管理 | 密钥写在脚本中，由开发人员管理 |
| 通道关联 | 通道和脚本没有直接关联关系 |
| 脚本职责 | 脚本代码中处理多个通道的逻辑 |

**业务流程**：
1. 运营人员在通道列表维护API地址、入参出参等业务参数
2. 开发人员编写脚本，处理加解密、认证等逻辑
3. 新增通道：先发布脚本（让脚本感知新通道），再新增通道
...
**说明**：
- `history`：版本历史，记录脚本的每次编辑
- `deployments`：各机房的发布状态，记录每个机房的当前版本
...
**标准流程**：
```
开发发布脚本 ──→ 测试环境验证 ──→ 宝山机房发布 ──→ 华东机房发布 ──→ 华北机房发布
                          ↓
                    测试不通过？
                          ↓
                    回滚到上一版本
...
4. 宝山机房通过后，发布到华东机房
5. 华东机房通过后，发布到华北机房
6. 某一机房测试不通过：回滚该机房到上一版本，其他机房不受影响

**关键点**：
- 测试不通过时，内容回滚到上一版本
- 多机房是独立处理的，每个机房可以有不同的版本
- 只有测试环境通过后，才发布到下一个机房
...
| 版本 | 备注 | 创建时间 | 创建人 | 操作 |
|------|------|---------|--------|------|
| v1.2.0 | 优化加解密逻辑 | 2026-04-10 14:30:00 | 张三 | 查看 |
| v1.1.0 | 支持多通道 | 2026-03-20 14:30:00 | 张三 | 查看 |

**说明**：
- 版本历史显示的是**创建时间**，不是发布时间
- 每次编辑保存会生成新的版本记录
- 不需要额外的"发布"动作，编辑即保存
...
| 机房 | 当前版本 | 发布时间 | 发布人 | 操作 |
|------|---------|---------|--------|------|
| 测试环境 | v1.2.0 | 2026-04-10 14:35:00 | 李四 | 发布 |
| 宝山机房 | v1.1.0 | 2026-04-09 10:00:00 | 李四 | 发布 |
| 华东机房 | v1.0.0 | 2026-04-08 09:00:00 | 王五 | 发布 |
| 华北机房 | - | - | - | 发布 |
...
| v1.2.0 | - | 新增脚本管理模块，支持多机房发布 |
| v1.2.1 | 2026-04-16 | 发现业务文档9处矛盾，记录到 docs/discussions/2026-04-16-业务文档待澄清问题.md |
