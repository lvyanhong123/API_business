...
在动手写代码之前，必须先完成业务梳理和模块划分。避免"边想边做"导致的模块边界不清、状态流转混乱问题。
...
| 文档 | 用途 | 粒度 |
|------|------|------|
| 业务流程图 | 清晰展示业务闭环、状态流转、模块边界 | 粗粒度（泳道图可选） |
| 功能模块划分 | 明确模块职责、接口、数据边界 | 按模块拆分 |
| 状态机/状态流转图 | 关键实体（订单/账户等）的状态变化 | 核心实体必做 |
...
**独立流程必须独立：**
```
❌ 错误：approveOrder 做了审核+扣款+创建Key+初始化配额
...
| 问题现象 | 根因 | 解决 |
|----------|------|------|
| 一个函数超过100行 | 职责不单一 | 拆分为多个函数 |
| 改一个功能影响多个地方 | 模块边界不清 | 重划模块边界 |
| 状态流转不清晰 | 缺少状态机图 | 先画状态流转图 |
| 代码"凑合能用" | 跳过设计直接写 | 强制前置流程图 |
