...
#### Git 上传流程（强制执行）
1. 分开执行命令，**禁止使用 `&&` 语法**（PowerShell 不支持）
2. 执行顺序：`git add -A` → `git commit -m "提交信息"` → `git tag v.x.x` → `git push` → `git push origin v.x.x`
3. 上传后**必须验证**：`git log` 和 `git tag -l` 确认提交和 tag 已同步
4. 如果验证失败，**立即重试**上传步骤
